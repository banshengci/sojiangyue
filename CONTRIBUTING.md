[English](#contributing-to-songjiang-reader) | [简体中文](#让松江阅更好) | [Русский](#вклад-в-songjiang-reader)

# Contributing to SongJiang Reader

SongJiang Reader is an open-source project, and we welcome any contributions from you. You can help by translating, fixing bugs, adding new features, writing documentation, and more. If you want to contribute, the following guide may be helpful.
...
### Translation
If you'd like SongJiang Reader to support your language, let's work together on the translation!
...
- SongJiang Reader uses [intl](https://pub.dev/packages/intl) for multilingual support. You can find the localization files in the `lib/l10n` directory. Please copy `app_en.arb` to `app_language_code.arb`, and then translate it.
...
SongJiang Reader uses [Flutter_inappwebview](https://pub.dev/packages/flutter_inappwebview) to render eBooks, so the project relies on `JavaScript` to handle eBook rendering.
...
# 让松江阅更好
松江阅是一款开源项目，我们欢迎您的任何贡献，您可以对项目进行翻译、修复 bug、添加新功能，编写文档等。如果您想要贡献，以下内容可能会对您有所帮助。
...
**翻译应用程序界面**
- 松江阅使用 [intl](https://pub.dev/packages/intl) 进行多语言支持，您可以在`lib/l10n`目录下找到多语言文件，请复制`app_en.arb`为`app_语言代码.arb`，然后进行翻译。
...
### 修复 bug 和添加新功能
松江阅使用 [Flutter_inappwebview](https://pub.dev/packages/flutter_inappwebview) 来渲染电子书，因此项目在渲染电子书的部分使用`JavaScript`编写。
...
# Вклад в SongJiang Reader

SongJiang Reader — это проект с открытым исходным кодом, и мы приветствуем любой ваш вклад. Вы можете помочь с переводом, исправлением ошибок, добавлением новых функций, написанием документации и многим другим. Если вы хотите внести свой вклад, следующее руководство может оказаться полезным.
...
### Перевод
Если вы хотите, чтобы SongJiang Reader поддерживал ваш язык, давайте поработаем над переводом вместе!
...
- SongJiang Reader использует [intl](https://pub.dev/packages/intl) для многоязычной поддержки. Файлы локализации находятся в каталоге `lib/l10n`. Пожалуйста, скопируйте `app_en.arb` в `app_код_языка.arb` и переведите его.
...
SongJiang Reader использует [Flutter_inappwebview](https://pub.dev/packages/flutter_inappwebview) для отображения электронных книг, поэтому рендеринг электронных книг в проекте реализован с помощью `JavaScript`.
